...
- [x] Добавить тест: строка с только температурой без акселерометра (`"Temp: 25.0 C"`) —
      `accel1X == null`.
- [x] Добавить тест: `"T=28.5"` (equals вместо colon) корректно парсится.
- [x] Добавить тест: отрицательные значения акселерометра (`"X: -128 Y: 0 Z: 64"`).
- [x] Добавить тест: строка с `\r\n` в конце не ломает парсинг (устройства на Windows-стиле).
- [x] Добавить тест: firmware CSV с отрицательной температурой (`"1;-5.5;-3.2;0,0,0;0,0,0;0;0;"`).
- [x] Добавить тест: bare-4 с comma-разделителем (`"28.50,254,0,59"`).
- [x] Запустить `./gradlew :app:testDebugUnitTest` — все тесты должны пройти.
...
- [x] `FftTest`: подать синусоиду частоты f при sampleRate S, убедиться что пик спектра
...
- [x] `Kalman1DTest`: подать два одинаковых измерения — оценка должна быть близка к ним.
...
- [x] `VibrationStatsTest`: подать массив нулей — RMS=0, Peak=0, Crest=NaN или 0.
      Подать чистую синусоиду — RMS = amplitude/√2 (погрешность <1%). Проверить Kurtosis:
      для гауссова шума ≈3, для импульсного — >3.
- [x] `CrossCorrelationTest`: подать два одинаковых сигнала с известным сдвигом N семплов,
      убедиться что `bestLag == N` и корреляция ≈1.0. Подать антифазные — корреляция ≈ -1.0.
- [x] `IntegrationTest`: подать константный акселератор 1.0 за 1 секунду (10 семплов по 0.1с),
...
- [x] `OrientationTest`: при `ax=0, ay=0, az=9.81` pitch=0°, roll=0°. При `ax=9.81` pitch≈-90°.
      Magnitude при `(1,0,0)` = 1.0.
- [x] `HeatFluxTest`: для меди k=400, T1=100, T2=50, d=0.01 — flux = -400*(50-100)/0.01 = 2_000_000.
